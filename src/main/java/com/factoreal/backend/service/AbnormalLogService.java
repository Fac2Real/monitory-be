...
public class AbnormalLogService {
    ...
    public Page<AbnormalLogDto> findAllAbnormalLogs(AbnormalPagingDto abnormalPagingDto) {
        ...
        return abnormalLogs.map(abn_log -> AbnormalLogDto.builder()
                .id(abn_log.getId())
    ...
    public Page<AbnormalLogDto> findAbnormalLogsByAbnormalType(AbnormalPagingDto abnormalPagingDto,
            String abnormalType) {
        // 한번에 DB전체를 주는 것이 아닌 구간 나눠서 전달하기 위함
        Pageable pageable = getPageable(abnormalPagingDto);
    ...
    //
    public Page<AbnormalLogDto> findAbnormalLogsByTargetId(AbnormalPagingDto abnormalPagingDto, String targetType,
            String targetId) {
        // 한번에 DB전체를 주는 것이 아닌 구간 나눠서 전달하기 위함
        Pageable pageable = getPageable(abnormalPagingDto);
